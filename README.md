...
HTTP协议请求(Request)分为3部分: 起始行(Start Line),首部(Header)以及实体主体(Body), 大致的解析原理是客户端请求进来后, 服务端读取请求的内容, 然后参照HTTP协议结构规范进行逐个部分解析

HTTP里头基本都是使用 '\r\n' 分隔, 解析起来也是挺......

HTTP结构就3部分, 起始行跟首部就逐行解析, 至于Body就依赖于 Content-Type类型解析
...
e.g.
application/json, text/plain...等一类的就直接将 Body 根据类型自行格式化, 转换就好了...
